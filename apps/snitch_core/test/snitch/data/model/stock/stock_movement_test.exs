defmodule Snitch.Data.Model.StockMovementTest do
  ...
  describe "create/4" do
    test "Fails for INVALID stock item id" do
      assert {:error, changeset} = Model.StockMovement.create(1, 1)
    ...
    test "Fails with invalid quantity", context do
      ...
      assert {:error, changeset} = Model.StockMovement.create("abc", stock_item.id)
      assert [quantity: {"is invalid", [type: :integer, validation: :cast]}] = changeset
    end

    test "Inserts with valid attributes", context do
      %{stock_item: stock_item} = context
      assert {:ok, stock_movement} = Model.StockMovement.create(1, stock_item.id)
  ...
  describe "get/1" do
    test "Fails with invalid id" do
      stock_item = Model.StockMovement.get(1)
    ...
    test "gets with valid id", context do
      ...
      get_stock_movement = Model.StockMovement.get(insert_stock_movement.id)
      ...
      # with stock item map
      get_stock_movement_with_map = Model.StockMovement.get(%{id: insert_stock_movement.id})
  ...
  describe "get_all/0" do
    test "fetches all the stock items" do
      stock_movements = Model.StockMovement.get_all()
      ...
      stock_movements_new = Model.StockMovement.get_all()
